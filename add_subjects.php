...
$sql_subjects = "
SELECT subject_id, subject_name, subject_code, department, credits 
FROM subjects"; 
                  ...
                  <div class="col-12">
                      <div class="card">
                          ...
                          <div id="addSubjectContainer" class="card-body" style="display: none;">
                              <form action="index_subjects.php" method="post">
                                  ...
                                  <tbody>
                                      ...
                                      if ($result_subjects->num_rows > 0) {
                                          while($row = $result_subjects->fetch_assoc()) {
                                              echo "<tr id='row_" . $row["subject_id"] . "'>";
                                              ...
                                              echo "<tr id='editForm_" . $row["subject_id"] . "' class='hidden-edit-form' style='display: none;'>
                                                <td colspan='5'>
                                                    <form action='index_subjects.php' method='post'>
                                                        ...
                                                        <div class='form-group'>
                                                            ...
                                                            <input type='text' class='form-control' name='subject_code' value='" . $row["subject_code"] . "'>
                                                        </div>
                                                        <div class='form-group'>
                                                            <label>Department</label>
                                                            <select class='form-control' name='department' required>";

                                                            // Query departments for each edit form
                                                            $sql_departments_edit = "SELECT dept_id, dept_name FROM departments";
                                                            $result_departments_edit = $conn->query($sql_departments_edit);

                                                            if ($result_departments_edit->num_rows > 0) {
                                                                while($dept_edit = $result_departments_edit->fetch_assoc()) {
                                                                    $selected = ($dept_edit['dept_name'] == $row['department']) ? 'selected' : ''; // Compare dept_name instead of dept_id
                                                                    echo "<option value='" . $dept_edit['dept_name'] . "' $selected>" . $dept_edit['dept_name'] . "</option>"; // Set value to dept_name
                                                                }
                                                            }
                                              ...
                                              echo "</select>
                                                        ...
                                                        <div class='form-group'>
                                                            <label>Credits</label>
                                                            <input type='text' class='form-control' name='credits' value='" . $row["credits"] . "'>
                                                        </div>
        ...
        tableBody.addEventListener('click', (event) => {
            if (event.target.classList.contains('deleteBtn')) {
                ...
                if (confirm('Are you sure you want to delete this subject?')) {
                    fetch(`index_subjects.php?delete_id=${rowId}`, {
                        method: 'DELETE'
                    }).then(response => {
                        if (response.ok) {
                            window.location.href = 'add_subjects.php?success=Subject deleted successfully';
                        } else {
                            response.text().then(text => {
                                const errorMessage = response.headers.get('X-Error-Message') || 'Error deleting subject';
                                window.location.href = `add_subjects.php?error=${encodeURIComponent(errorMessage)}`;
                            });
                        }
                    }).catch(error => {
                        window.location.href = `add_subjects.php?error=${encodeURIComponent('Error deleting subject')}`;
                    });
                }
            }
        });
